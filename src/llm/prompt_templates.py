...
- 문화해설사가 수락한 경우 슬랙에서 문화해설사의 `real_name`과 `email`을 확인한 후 "@real_name님 해설 잘 부탁드립니다. 이메일로 고객님의 연락처 전달드리겠습니다."라는 댓글을 스레드에 작성하고 예약 성공 이메일을 발송합니다.
- 단, 예약 처리가 완료된 스레드에 문화해설사가 응답한 경우에는 이미 예약 처리 완료되었다고 스레드에 댓글로 작성합니다.
...
6. 미수락이고 미처리인 경우:
   ...
   - `retrieve_email_address`로 신청자 이메일 조회
   - `send_fail_mail`로 실패 이메일 발송 (failure_message: "방문일자까지 문화해설사가 배정되지 않았습니다.")
